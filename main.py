...
def calendar():
    ...
    for key in sorted(cal,reverse=True):
        ...
        #meta_url = "plugin://%s/tv/search_term/%s/1" % (plugin.get_setting('catchup.plugin').lower(),urllib.quote_plus(title))
        if tvdb[imdbID]:
            meta_url = "plugin://%s/tv/play/%s/%s/%s/%s)" % (plugin.get_setting('catchup.plugin').lower(),tvdb[imdbID], season,episode, "default")
        else:
            meta_url = "plugin://%s/tv/play_by_name/%s/%s/%s/%s)" % (plugin.get_setting('catchup.plugin').lower(),title, season,episode, "en")
